    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md p-6 relative">
        ...
        <div className="mt-4 flex flex-col items-center gap-2 text-xs text-slate-500">
          {mode === "signin" && (
            <>
              <button onClick={() => { setMode("signup"); setError(null); }} className="hover:text-slate-300">
                No account? Sign up free
              </button>
              <button onClick={() => { setMode("reset"); setError(null); }} className="hover:text-slate-300">
                Forgot password?
              </button>
            </>
          )}
          {mode === "signup" && (
            <button onClick={() => { setMode("signin"); setError(null); }} className="hover:text-slate-300">
